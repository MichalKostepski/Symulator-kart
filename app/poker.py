...
class PokerGame(Game):
    ...
    def add_bot(self):
        with self.lock:
            ...
            self.players.append((player_id, None, None))
            self.game_state["chips"][player_id] = self.STARTING_CHIPS
            self.bots[player_id] = PokerBot(self, player_id)

            print(f"[POKER] Bot {player_id} dołączył")
            return player_id

    def run_bot(self, player_id):
        bot = self.bots.get(player_id)
        if bot is None:
            return

        try:
            msg = bot.create_move_message()
        except Exception as error:


            print(f"[POKER BOT ERROR] Bot {player_id}: {error}")
            msg = self.create_msg(
                game="POKER",
                msg_type="MOVE",
                player_id=player_id,
        ...
        print(f"[POKER BOT] Ruch bota {player_id}: {msg}")
    ...
    def can_player_act(self, player_id):
        return (
            player_id not in self.game_state["folded_players"]
            and self.game_state["chips"].get(player_id, 0) > 0
        )
    ...
    def get_next_active_turn(self, start_index):
        ...
        while checked < len(self.players):
            ...
            if self.can_player_act(next_player_id):
                return next_index
    ...
    def send_turn_to_current_player(self):
        ...
        if turn_player_id in self.bots:
    ...
    def send_table_update(self):
        msg = self.create_msg(
            ...
            data={
                ...
                "round_bets": self.game_state["round_bets"],
                "current_turn_index": self.game_state["current_turn"],
                "current_turn_player_id": (
                    self.players[self.game_state["current_turn"]][0]
                    if self.game_state["current_turn"] is not None
                    and 0 <= self.game_state["current_turn"] < len(self.players)
                    else None
                ),
                "blind_player_id": (
                    self.players[self.game_state["blind_player"]][0]
                    if self.players
                    and 0 <= self.game_state["blind_player"] < len(self.players)
                    else None
                ),
                "players": [
                    {
                        "player_id": player_id,
                        "is_bot": player_id in self.bots,
                        "chips": self.game_state["chips"].get(player_id, 0),
                        "bet": self.game_state["round_bets"].get(player_id, 0),
                        "folded": player_id in self.game_state["folded_players"],
                        "all_in": (
                            player_id not in self.game_state["folded_players"]
                            and self.game_state["chips"].get(player_id, 0) == 0
                        ),
    ...
    def is_betting_round_over(self):
        ...
        for pid in players_who_can_act:
    ...
    def advance_phase_locked(self):
        ...
        if self.game_state["current_turn"] is None or self.is_betting_round_over():
            self.send_table_update()
    ...
    def handle_message(self, msg):
        with self.lock:
            ...
            if next_turn is None:
                print("[INFO] Brak gracza mogącego wykonać ruch — przechodzę dalej")
                self.send_table_update()
                self.advance_phase_locked()
                return
    ...
    def handle_bancruptcies(self):
        bankrupts = [pid for pid, chips in list(self.game_state["chips"].items()) if chips <= 0]
        for pid in bankrupts:
            ...
            self.game_state["chips"].pop(pid, None)
            self.game_state["round_bets"].pop(pid, None)
            self.game_state["hands"].pop(pid, None)
            self.game_state["folded_players"].discard(pid)
            self.game_state["players_acted"].discard(pid)
